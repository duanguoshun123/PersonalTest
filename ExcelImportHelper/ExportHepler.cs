...
namespace ExcelImportHelper
{
    public class ExportHepler
    {
        ...
        /// <returns></returns>
        public static MemoryStream CreateExcelStreamByDatas<T>(List<KeyValuePair<List<T>, KeyValuePair<string, string>>> objectDatas, string xmlPaths)
        {
            // 返回对象
            var ms = new MemoryStream();
            ...
            {
                //导入数据到sheet表单
                CreateExcelSheetByDatas(keyValuePair.Key, keyValuePair.Value.Key, keyValuePair.Value.Value, ref workbook, xmlPaths);
            }
        ...
        public static MemoryStream ExportDataTableToExcel(DataTable sourceTable)
        {
            ...
            for (int i = 0; i < dtRowsCount; i++)
            {
                ...
                XSSFRow dataRow = (XSSFRow)sheet.CreateRow(tempIndex);
                foreach (DataColumn column in sourceTable.Columns)
                {
